...
def loop_abm(params,
    ...
    for t in range(T):
        ...
        if nfree == 0 and quarantine_HH:
            rank_algo = [(i, rng.random()) for i in range(N)]
        else:
            rank_algo = inference_algorithm.rank(t, weighted_contacts, daily_obs, data)

        rank = [int(i) for i, _ in sorted(rank_algo, key=lambda x: x[1], reverse=True)]
